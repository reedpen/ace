# -*- coding: utf-8 -*-
"""
Created on Fri Aug 14 11:25:19 2020

@author: Eric

Run calcium imaging analysis on select ranges of miniscope recordings from 
select experiments.
"""

import time
start=time.time()
import os
# import glob
import miniscope
import numpy as np

import sys
jobID = ''
if len(sys.argv) > 1:
    jobID = sys.argv[1] + '_'

lineNums = [83,85,86,87,88,90,93,95,97,103]
movieNums = [[*range(213,229+1)],
             [*range(66,103+1)],
             [*range(66,89+1)],
             [*range(87,125+1)],
             [*range(69,125+1)],
             [*range(152,182+1)],
             [*range(135,169+1)],
             [*range(58,104+1)],
             [*range(66,135+1)],
             [*range(75,140+1)]]

for j, lineNum in enumerate(lineNums):
    for k in movieNums[j]:
        obj = miniscope.UCLAMiniscope(lineNum=lineNum, jobID=jobID)
        
        #%% Import movies
        movieFilenameAfterNum = '.avi'
        obj.movieFilePaths = os.path.join(obj.experiment['calcium imaging directory'], 'Miniscope', str(k) + movieFilenameAfterNum)
        
        # obj.importCaMovies(obj.movieFilePaths)
        
        #%% Crop and re-save the videos
        # obj.preprocessCaMovies(saveMovie=False, crop=True, cropGUI=True)
        
        # for k in movieList:
        #     obj.importCaMovies(k)
        #     obj.preprocessCaMovies(saveMovie=True, crop=True, cropGUI=False)
        
        #%% Run motion correction and CNMF-E and save the estimates object
        obj.processCaMovies(parallel=False, n_processes=8, saveCNMFEFilename='cnm_'+str(lineNum)+'_'+str(k)+'.estimates.hdf5')
        
        print('obj.movieFilePaths = ' + str(obj.movieFilePaths))
        print('cnm_'+str(lineNum)+'_'+str(k)+'.estimates.hdf5')
        
        #%% Delete memmapped files
        # # Get a list of all the file paths that ends with .txt from in specified directory
        # if type(obj.movieFilePaths) is str:
        #     fileList = glob.glob(os.path.split(obj.movieFilePaths)[0] + '/*.mmap')
        # else:
        #     fileList = glob.glob(os.path.split(obj.movieFilePaths[0])[0] + '/*.mmap')
        # # Iterate over the list of filepaths & remove each file.
        # for filePath in fileList:
        #     try:
        #         os.remove(filePath)
        #     except:
        #         print("Error while deleting file : ", filePath)

stop=time.time()
print(str((stop-start)/60))