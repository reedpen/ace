...
def test_neuralynx_can_handle_empty_directory(tmp_path):
    assert NeuralynxDataManager.can_handle(tmp_path) is False
...
@patch("ace_neuro.shared.experiment_data_manager.ExperimentDataManager.__init__")
@patch("ace_neuro.shared.experiment_data_manager.ExperimentDataManager.get_miniscope_directory")
def test_miniscope_autodetect_empty_dir_raises(mock_get_dir, mock_init, tmp_path):
    mock_init.return_value = None
    mock_get_dir.return_value = str(tmp_path)
    with pytest.raises(ValueError, match="No MiniscopeDataManager subclass"):
        MiniscopeDataManager.create(line_num=1, auto_import_data=False)


def test_ephys_autodetect_empty_dir_raises(tmp_path):
    with pytest.raises(ValueError, match="No EphysDataManager subclass"):
        EphysDataManager.create(
            ephys_directory=tmp_path,
