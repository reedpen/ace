...
#display a gui to manually crop
miniscope_data_manager = MiniscopeDataManager(line_num, filenames = ['0.avi', '1.avi'])
...
coords_dict, _ = get_coords_dict_from_analysis_params(miniscope_data_manager, crop_square=True)
movie, crop_coords = preprocessor.crop_movie(miniscope_data_manager.movie, coords_dict=coords_dict, projections=projections)

#save those coords under crop_square
updateCSVCell(crop_coords, 'crop_square', line_num, ANALYSIS_PARAMS)

#clean up the coords
crop_coords = crop_coords.strip('()').split(',')
...
#load each movie into memory and crop
for movie_filepath in movie_filepaths:
    print(movie_filepath)
    miniscope_data_manager.movie = cm.load(str(movie_filepath))
    miniscope_data_manager.movie = miniscope_data_manager.movie[:, y0:y1, x0:x1]
    cm.play_movie(miniscope_data_manager.movie)
    time_projection = miniscope_data_manager.movie.mean(axis=(1,2))
