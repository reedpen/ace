...
class miniscope(experiment.experiment):
    ...
    def _componentImage(self, graph, max=False, min=False, STD=False, mean=False, median=False, range=False,
        ...
        if max:
            ...
            # FIXME try replacing self.projections['Max'] with self.estimates.plot_contours(img=self.projections['Max']) or the image generated by that in order to graph the image
        elif min:
            plt.imsave(pic_IObytes, self.projections['Min'], format='png', cmap=cmap)
        elif STD:
            plt.imsave(pic_IObytes, self.projections['Std'], format='png', cmap=cmap)
    ...
    def _componentGUI(self):
        ...
        while True:
            ...
            # CMAP of image
                ...
                # FIXME listbox.get here to get all of the selected data points from the list box
                # FIXME then handle either deleting the components
                break
